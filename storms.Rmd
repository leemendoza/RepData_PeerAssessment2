...
```
## Results  
The rankings of the `r nrow(mostharmful)` most harmful weather events across the US are shown below. As you can see, the event resulting in the most fatalities is `r mostharmful[1, 1]`, the most injuries is `r mostharmful[1, 3]`, the most property damage is `r mostharmful[1, 5]`, and the most crop damage is `r mostharmful[1, 7]`
...
grid.arrange(p1, p2, ncol=1, nrow=2, sub = textGrob("Events most harmful to population health"))
```
```{r fig.width=12, fig.height=12, fig.align='center'}
